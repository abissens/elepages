...
mod tests {
    ...
    fn generate_index_pages_from_single_page_bundle() {
        ...
        assert_eq!(
            ...
            BundleIndex {
                ...
                pages_with_metadata: vec![PageIndex {
                    page_ref: PageRef {
                        path: vec!["dir".to_string(), "f1".to_string()]
                    },
                    page_uri: "/dir/f1".to_string(),
    ...
    fn query_bundle_index() {
        ...
        assert_eq!(result, bundle_index.pages_with_metadata.iter().collect::<Vec<&PageIndex>>());
        ...
        assert_eq!(result, vec![bundle_index.pages_with_metadata.get(0).unwrap(), bundle_index.pages_with_metadata.get(2).unwrap()]);

        let result = bundle_index.query(&TagQuery("t1".to_string()), &BundlePagination { skip: None, limit: None });
        assert_eq!(result, vec![bundle_index.pages_with_metadata.get(0).unwrap()]);
    ...
    fn paginate_bundle_index_query() {
        ...
        assert_eq!(result, vec![bundle_index.pages_with_metadata.get(0).unwrap(), bundle_index.pages_with_metadata.get(1).unwrap()]);

        let result = bundle_index.query(&AlwaysQuery, &BundlePagination { skip: Some(1), limit: Some(1) });
        assert_eq!(result, vec![bundle_index.pages_with_metadata.get(1).unwrap()]);

        let result = bundle_index.query(&AlwaysQuery, &BundlePagination { skip: Some(2), limit: None });
        assert_eq!(result, vec![bundle_index.all_pages.get(3).unwrap()]);
    }
}
